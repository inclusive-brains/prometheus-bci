...
document.addEventListener('DOMContentLoaded', function () {
        ...
        for (var timestamp in data) {
            for (var metric in data[timestamp]) {
                if (!chartsMetrics[metric]) {
                    ...
                    chartsMetrics[metric] = new SmoothieChart(Object.assign({}, darkChartOptions, {
                    ...
                    chartsMetrics[metric].addTimeSeries(timeSeriesMetrics[metric], {
                        lineWidth: 1.5,
                        strokeStyle: '#7ab3bd',
        ...
        for (var timestamp in data) {
            for (var metric in data[timestamp]) {
                if (!chartsBlendshapes[metric]) {
                    ...
                    chartsBlendshapes[metric] = new SmoothieChart(Object.assign({}, darkChartOptions, {
                        title: { fillStyle: '#8b7db5', text: metric, fontSize: 12, fontFamily: "'JetBrains Mono', monospace", verticalAlign: 'top' }
                    }));
                    ...
                    chartsBlendshapes[metric].addTimeSeries(timeSeriesBlendshapes[metric], {
                        lineWidth: 1.5,
                        strokeStyle: '#8b7db5',
                        fillStyle: 'rgba(167,139,250,0.05)',
                        interpolation: 'bezier'
                    });
                }
    ...
    io.on('multimodal_attention', (data) => {
        ...
        if (robot) {
            ...
            var threshold = parseFloat(document.getElementById('thresholdInput')?.value || 0.8);
